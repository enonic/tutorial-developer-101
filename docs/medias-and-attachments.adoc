...
This differs from the previously presented media content, which is a separate content itself that can be linked on other content types, for example, by using the https://developer.enonic.com/docs/xp/stable/cms/input-types#contentselector[ContentSelector] input type.

To take a look on how attachments work, let's update our Artist content type to use the input type _AttachmentUploader_. 
...
+
. Redeploy your app
. Add some attachments to each of our three artists. In my case I'll add images of some of their albums:
+
...
With that, we have not only enhanced our Artist content type with a new structure, but also added files as attachments to all of our three artists.
...
Let's understand how we can retrieve our artists attachments through queries.

First, access the GraphQL API docs on query playground and search for the `Content` schema.  Scroll down a bit and you'll note that there is a field name _attachments_. Now look for the `com_example_myproject_Artist_Data` schema. You'll also note that, after our changes on the Attachments section, we also have a _attachments_ field there, of the same type _Attachment_ of the previous one
