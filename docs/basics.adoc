...
image::xp-tour-1.png[A popup describing the tools and navigation system of Enonic XP.]

.The second step of the welcome tour
image::xp-tour-2.png[A description of XP applications and the Enonic Market.]

.The final step of the welcome tour
image::xp-tour-3.png[A prompt to install a set of applications from the Enonic Market including Content Studio, Live Trace, and Data Toolbox.]
...
In this {document} we've had our first glimpse at using the Enonic CLI,  we've had a little tour of the XP Admin console, and we've installed some XP apps to prepare for later.

=== Useful links

== Next

In the next {document} we'll use the sandbox that we've just created and create our first XP application and see how we can get an automatically generated GraphQL API for content that we create.
