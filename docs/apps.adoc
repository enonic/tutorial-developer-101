...
. Make sure you are in the directory in which you want your project to be created and run `enonic create -r starter-vanilla {project-name}`;
. The first arugment after `-r` is the starter, ir our case, `starter-vanilla`;
. The second arugment after `-r` is the name of the folder that will be created containing the structure and the files you'll be working on;
