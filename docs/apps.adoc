...
To clarify on the previous claims, here are some examples of XP apps... if you want have analytics, you can install https://market.enonic.com/vendors/enonic/google-analytics[google analytics app]. If you want to use Enonic XP CMS in a headless way, you can install https://market.enonic.com/vendors/enonic/guillotine[guillotine app], and access the created content through a GraphQL API. At the same time, your project idea will most likely be an app.
...
. Make sure you are in the directory in which you want your project to be created and run `enonic project create`;
. Use kbd:[Up] and kbd:[Down] to navigate between the options and chose the `Vanilla starter`;
. Keep pressing kbd:[Enter] to continue with the default values for project name, destination folder and project version;
. Make sure to select the previously created sandbox when asked;
...
This copies the application .jar file to your sandbox's `home/deploy/` folder. Since in this case we deployed `{project-name}.jar` into `tutorial` sandbox, we should expect that file to be on `~/.enonic/sandboxes/tutorial/home/deploy`.
