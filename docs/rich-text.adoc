...
This rich text input type offers some very powerful editing capabilities and a lot of configuration options. 

This while {document} is dedicated to https://developer.enonic.com/docs/xp/stable/cms/input-types#htmlarea[HtmlArea input type] and the Rich text editor used to define its data. For a full detailed exaplanation of the Rich text editor used in content studio, please visit the https://developer.enonic.com/docs/content-studio/stable/editor/rich-text-editor[Rich text editor reference documentation].
...
The embed macro enable you to insert iframes. In our context, we'll embed this https://www.youtube.com/watch?v=gr5EwZo_CHQ[Lynx youtube video].
