...
.Using a panel macro
[src]
  [success header="Congrats!"]You did it! Give yourself a well-deserved pat on the back 🥳[/success]
...
Now make sure this is deployed to your XP instance (check xref:basics#_sandboxes[the section on sandboxes in for how to start it]) either by having it run in development mode or by xref:basics#_building_and_deploying[redeploying your app].
...
----
{
  guillotine {
    ...
    {
      displayName
      ... on com_gettingstarted_content_{content-type-2-capitalized} {
        data {
          name
