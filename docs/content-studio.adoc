...
image::app-vs-projects-flow.png[Simplied flow explaining the difference between an Enonic XP project folder and its files, an Enonic XP app, and a Content Studio project, 300, align=center]
...
Back to your preferred IDE, make sure you're in the project folder (`{project-name}/`) that you created in a previous {document}. All paths will be relative to this root folder.
...
Since we'll be using this project in the coming {document}s with more content types, having some hierarchy will make things easier. Choose a new folder and call it "{content-type-1}s".
