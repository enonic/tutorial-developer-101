...
NOTE: This command will watch changes under `{project-name}/` and redeploy as needed. Make sure you're already running your sandbox with the `--dev` flag in another terminal, otherwise running enonic project dev will start the sandbox in detached mode, so you'll lose logs. More on cli's dev command https://developer.enonic.com/docs/enonic-cli/stable/dev#dev[here]. 
