...
Enonic is a hybrid CMS, which means that it can be used to manage and deliver content to our frontend. 
...
As an example on how to retrieve some minimal info about our previously created artists, run the following query:
...
image::query-playground-buttons.png[Query playground buttons labelled for further detailing, {image-xl}]
...
Note that it's said that the `Artist` schema _implements_ the `Content` schema. This basically means that our `Artist` schema will have the structure defined by the `Content` schema, together with possibly some additional structure... in our case, this additional structure is a `data` field, defined by `com_example_myproject_Artist_Data` schema:
...
NOTE: Note that this matches the structure defined by our Artist content type, which is a natural thing to expect.  

=== Querying with inline fragments

We've used the Docs section to inspect the structure of our `Artist` schema. Let's now, based on our understanding of this schema, query to get proper data from our artists.
...
As can be seen, we do not have the artist `name` and `about` fields available, and the reason for that is because the `getChildren` field can return any content that implements the `Content` schema, but it can't directly infer what schema that is, which in our case is the `com_example_myproject_Artist` schema:

.getChildren can't directly infer what schema is implementing Content schema
image::query-playground-getChildren-infer.png[Query playground getChildren can't directly infer what schema is implementing Content schema, {image-l}]
...
Just like there is a `query` field underneath `guillotine`, our API also provides other fields such as `queryConnection`, `queryDsl` and `queryDslConnection`.

We'll not go into much details, but basically, the fields with _connection_ in their name have new parameters for aggregating data, whereas in the fields with _dsl_ (doman specific language), the query argument is not a string bot our own specific DSL.

To make it clear, we'll rewrite our last query that used `ngram` using the queryDsl field:
...
NOTE: If you've previously used Elasticsearch API to query data, you'll be quite familiar with how our `queryDsl` works!
...
To finish this {document}, we'll add a new _birthyear_ field to our artist content type, update all artist to have data on this field, and then use `queryDslConnection` field to get aggregated data. Let's get to work!
...
[start=2]
. Run `enonic project deploy` in the terminal on the root directory of the project
...
. Go back to Query playground and click on the _re-fetch schema button_. This will update our Artist data schema to contain the birthday structure, which is in sync with our Artist content type structure
...
Which shows that two of our artists are from `Generation X`, and one of them is a `Millenial`.
