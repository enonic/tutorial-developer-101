...
Enonic is a hybrid CMS. As a hybrid CMS, Enonic is both capable of managing content, and support customized delivery. In this {document} we will focus on delivery via the out-of-the-box GraphQL API.
...
+
image::query-playground-button-1.png[Query playground play button, width=75]
. _play button_, will execute your query. Shortcut is kbd:[ctrl+enter]

+
image::query-playground-button-2.png[Query playground prettify button, width=75]
...
+
image::query-playground-button-3.png[Query playground merge button, width=75]
...
+
image::query-playground-button-4.png[Query playground copy button, width=75]
. _copy button_, will copy your query to your clipboard. Shortcut is kbd:[ctl+shift+c]

+
image::query-playground-button-5.png[Query playground docs button, width=75]
. _docs button_, will open a section showing all details about the schemas you have avaiable to use and query on

+
image::query-playground-button-6.png[Query playground history button, width=75]
. _history button_, will open a section showing all previously executed queries

+
image::query-playground-button-7.png[Query playground tab button, width=75]
. _tab button_, will create new tabs for you to manage testing multiple queries

+
image::query-playground-button-8.png[Query playground draft and master buttons, width=200]
. _draft / master buttons_ will switch between those two repositories for your queries to run on

+
image::query-playground-button-9.png[Query playground re-fetch button, width=75]
. _re-fetch schema button_, will update your graphql schemas based on recent changes

+
image::query-playground-button-10.png[Query playground shortkeys button, width=75]
. _shortkeys button_, will open a dialog displaying all short keys that can be used to execute actions

+
image::query-playground-button-11.png[Query playground settings button, width=75]
