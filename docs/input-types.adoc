...
== Task: Create an {content-type-2} data set
...
To verify that everything works, create some animals, and try creating a content link between a person and an animal.

When you're ready, create a GraphQL query that'll fetch all people and list what their favorite animals are, as well their animals' descriptions .
