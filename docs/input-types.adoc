...
All input types share a number of options that affect how they act ...

// use textline to illustrate here
...
image::input-type-text-line.png[TextLine: A single-line input field., {image-m}]
...
Content Studio's comes with built-in image editing facilities where you can edit the image by cropping, rotating, or flipping it. You can also add an image focal point, so that if the image gets cropped for some reason, you're in control of what part of the image remains visible.
...
After you've added an image (or several), you can highlight it and click the edit button. This will take you to the image editing form. Here, you can edit the image and image metadata, including (but definitely not limited to) image alt text, caption, copyright information, location data, and much more.

For image editing, you can crop, flip, and rotate the image, as well add a focal point. The latter is an important tool for images that may display at different ratios depending on screen sizes. When an image gets fitted to a container, it's easy for the important part of the image to get cropped out. Content Studio solves this by letting you specify a focal point manually.

.Setting the focal point
image::image-editing-focal-point.png[An image with a focal point around the key part of the image, {image-xl}]


NOTE: If you'd like to lean more about image editing, https://enonic.com/blog/enonic-simplifies-image-problems[blog post on how Enonic simplifies your everyday image problems] has lots of great information about Content Studio's image handling capabilities.
...
. An image selector that accepts between 0 and 3 images. You should also add at least one image and try out the XP image editing tools. Make sure the focal point is on the {content-type-2}s head.
...
What about fetching the images on the {content-type-2}s? Let's look at a slightly more advanced query. Use the display name for one of the {content-type-2}s that you uploaded an image to to filter the content types you're interested in. Then, specify that you only want the first image in the list. This query is a bit more advanced that what we've seen so far, so it deserves a little bit of explanation:
...
{
  guillotine {
    query(contentTypes:"com.example.myproject:{content-type-2}",
...
<.> Using the `query` parameter, we can specify a query string to search only for specific content. The query uses Enonic's node query language (NoQL). The  https://developer.enonic.com/docs/xp/stable/storage/noql[NoQL reference docs] have a very detailed overview over the syntax if you're interested.
<.> Because `images` is a list, we specify that we're only interested in the first element.
<.> `imageUrl` requires you to specify a `scale` function. In this case, we're asking for a picture that's 400 pixels wide. The height will be depend on the image's aspect ratio. For more info on scaling, see https://developer.enonic.com/docs/xp/stable/runtime/engines/site-engine/image-service#scaling_methods[the scaling methods documentation].
<.> Where `imageUrl` gets a scaled and cropped version of the image, `mediaUrl` points you to the raw file instead.


Using this query, you'll end up with something like this (though naturally, your id's are likely to look a little different):
      ...
      "query": [
        {
          ...
          "data": {
            "images": [
              {
                "displayName": "female-lion",
                "imageUrl": "http://localhost:8080/site/default/draft/my-first-site/_/image/238ba006-f596-46ef-b404-6459c4c774b5:44fd5b74270e5261b5852cabbb89b4bf588aaf35/width-400/female-lion.jpg",
                "mediaUrl": "/site/default/draft/my-first-site/_/attachment/inline/238ba006-f596-46ef-b404-6459c4c774b5:810ee47b2966db876c658635d0060a19b8d5f57a/female-lion.jpg"
              }
            ]
          }
        }
