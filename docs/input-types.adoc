...
When you've created the content type, create a new folder in Content Studio at next to the people folder. Call it "{content-type-2}s".
...
When you've got that, create a GraphQL query that'll fetch all people and list what their favorite animals are, as well their animals' descriptions .
...
Now create some links using ContentSelector from people to {content-type-2}s. The important part here is that you link some content so that we can fetch it using GraphQL. When you have created some links, let's move on to the query.

The way to link to linked content in GraphQL is to add it as a field as part of the data. This query will fetch all people and their linked {content-type-2}s:

.Query to fetch people with their associated favorite animals
[source,GraphQL]
----
{
  guillotine {
    getChildren(key: "/my-first-site/people/")
    {
      displayName
      ... on com_gettingstarted_content_Person {
        data {
          name
          alias
          favoriteAnimal {
            displayName
            ... on com_gettingstarted_content_Animal{
              data {
                name
                description
              }
...
.The list of people with their favorite animals
[source,json]
----
{
  "data": {
