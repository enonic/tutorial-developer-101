...
<.> This allows us to query properties that belong to the `media:Image` GraphQL type.
<.> The `mediaUrl` property (also available on all other media types) gives you a URL from where the raw file can be downloaded.
