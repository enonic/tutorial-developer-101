...
Naturally, XP can also store any files you would want to serve or associate with any data. This {document} explores how XP stores uploaded data and focuses on how its image tools work. At the end of the chapter we'll walk through how to use images in a headless fashion.
...
<.> The `mediaUrl` property (also available on all other media types) gives you the relative path (from the server) to the media
