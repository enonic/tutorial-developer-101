...
As the name implies, the filters argument lets you apply filters to the image. Use filters to apply coloration functions and to rotate and flip images on the fly. https://developer.enonic.com/docs/xp/stable/runtime/engines/site-engine/image-service#filters[The filters documentation] has an exhaustive list of the available filters.

For instance, we could get all the images under the `images` folder, scaled to 750x600 pixels and in grayscale with a query like this:
...
<.> This allows us to get properties that belong to the `media:Image` type.
  ...
  "data": {
    "guillotine": {
      "getChildren": [
        {
          "displayName": "lion",
          "imageUrl": "http://localhost:8080/site/default/draft/my-first-site/_/image/3443336b-71e3-40a8-9a04-bc59eae934f8:7e7185830ee56f651aee49bf4aa98231ff8c1ecb/block-750-600/lion.jpg?filter=grayscale%28%29"
        }
      ]
    }
  }
