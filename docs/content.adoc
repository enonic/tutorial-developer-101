...
// TODO: consider removing the "name" property. It gets doubled up when using display name and there's no good reason to do this at this stage.
...
<4> This is the first property of the color. The `type` attribute specifies what kind of input field the user will see in Content Studio, and the `label` specifies how the input will be labeled.
...
To confirm and return to the Content Studio hierarchy, you can either press "Save" in the menu bar or use the keyboard shortcut kbd:[ctrl+enter]. The keyboard shortcut will also close the tab for you after saving. If you clicked the "Save" button in the menu, feel free to close the tab manually (you won't be needing it again).
...
. Use "red" for the name field and "#ff0000" in the "hexadecimal representation" field.

It should look a little something like this:

.The color form filled out for the color "red"
image::content-studio-new-red.png[The form for "red" fully filled out.]

Again, save and close the tab (kbd:[ctrl+enter]). Now repeat this process for "green" (hexadecimal code: "#00ff00") and "blue" (hexadecimal code: "#0000ff"). Be aware that Content Studio lets you nest content, so if you try and create new content when "red" is selected in the hierarchy, it'll nest the new content under "red". In our case, though, we want the colors to be nested directly under the site, so make sure "colors" is selected when you create new content.

NOTE: Notice how we end use the color name both as the display name of the content and in the content's "name" field? XP actually has a way to automate that for us! We can use something called https://developer.enonic.com/docs/xp/stable/cms/content-types#display_name_expressions[_display name expressions_]. It's not necessary for our simple use case here, but if you want to look into it, feel free to play around a bit.
...
image::content-studio-with-colors.png[The content hierarchy showing the colors site with "green", "blue", "red", and "Templates" listed directly underneath it.]

== Querying the API

So we've created a custom content type and used it to create content, but what good is content if we can't access it from anywhere? Luckily, this headless starter kit has our backs! When using the headless starter kit, we get a GraphQL API automatically generated without any additional work.

There's no way to find out, but http://localhost:8080/site/default/draft/colors/api

// where do you find the url to get to the API?
