...
:api-url: {xp-url}/site/default/draft{p1-site-path}/api
:content-type: person
:content-type-capitalized: Person
:subs: "attributes+"
...
Imagine we want our app to list people. We'll create a simple representation of a {content-type}: we're only interested in their name and an optional alias, nickname, or pseudonym. Here's what a {content-type} content type could look like. The most relevant parts are numbered and further explained below.


.A simple content type describing the '{content-type}' content type
[source,xml,subs={subs}]
----
<?xml version="1.0" encoding="utf-8"?>
<content-type>
  <display-name>{content-type-capitalized}</display-name> <!--1-->
  <description>A description of a {content-type}</description> <!--2-->
...
For instance, if we created an entry for Belcalis Marlenis Almánzar, better known as Cardi B, then that instance of the {content-type} content type (that piece of content) would be represented in JSON as this:

.A piece of content of type '{content-type-capitalized}'
[source,json]
----
{
...
Now that we know what a content type is, let's look at how we create them and make XP recognize them. We'll use the "{content-type-capitalized}" content type from before to keep things simple.
...
Within the `content-types` directory, we need to place another directory with the name of the content type (in our case: "{content-type-capitalized}"), and within that directory, we place our content type, `{content-type-capitalized}.xml`. As such, the full path to our content type from the project root is `src/main/resources/site/content-types/{content-type}/{content-type}.xml`.
...
In the `{content-type}.xml` file, place the following contents:

.The {content-type-capitalized} content type
[source,xml]
----
<?xml version="1.0" encoding="utf-8"?>
<content-type>
  <display-name>{content-type-capitalized}</display-name>
  <description>A description of a {content-type}</description>
  <super-type>base:structured</super-type>
  <form>
...
With the "{p1-site-name}" site highlighted, press "New ..." (or kbd:[Alt+n]!) to open the "Create Content" menu again. This time around, you should have some more choices than what you did previously, including our very own content type "{content-type-capitalized}"!

.The "Create Content" dialog with more content types
image::content-studio-new-content.png["The \"Create Content\" dialog when creating content under the \"{p1-site-name}\" site. It contains more elements than previously, including the \"{content-type-capitalized}\" content type that we created earlier."]
...
.The content form filled out for the {content-type} "Cardi B"
image::content-studio-new-{content-type}.png[The form for "{content-type}" fully filled out.]
...
----
{
  guillotine {
    ...
    {
      displayName
      ... on com_gettingstarted_content_{content-type-Capitalized} {
        data {
          name
          alias
        }
