...
Let's add an item set to the `{content-type-1-capitalized}`. Our goal will be to add a Filmography item set, featuring where and when a specific artist was featured in a movie.
...
- Because each distribution option should have the same fields, create a _mixin_ to reuse it.
