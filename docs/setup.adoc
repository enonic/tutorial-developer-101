...
Before we can start building, let's make sure we have the right tools available to work efficiently with XP. This {document} walks you through installing the fundamental tools we will be using throughout this guide, including a text editor, a package manager for your system, Git, and the Enonic CLI. 
...
. Make the `enonic/cli` repository via https://docs.brew.sh/Taps[Homebrew's tap functionality]
...
. Make the CLI https://scoop.netlify.app/concepts/#buckets[bucket] available to install
